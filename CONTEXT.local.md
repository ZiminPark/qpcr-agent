...
- **작업 목록 (worklist)**: "어디서 → 어디로 → 몇 µL" 목록. 2단계 승인 화면의 주인공. worklist는 실제 업계 용어지만 화면·대사·에러 메시지에는 "작업 목록"만 쓴다 — `worklist`는 `run_liquid_handler` 파라미터명 등 코드 식별자로만.
...
- **시약 잔량 (reagent_ul)**: buffer 잔량 숫자 하나 (µL 단위 — 결정 19). stage5 사고 데모용.
- **GAPDH / IL6**: one-step RT-qPCR의 2채널 multiplex assay로 같은 well에서 함께 측정하는 기준 유전자 / 목표 유전자. 데모에서는 IL6 형광 곡선과 장비가 계산한 GAPDH Cq만 노출한다.
