...
가상 실험실을 붙여가며 챗봇을 "qPCR Agent"로 키우는 라이브 데모입니다. 복붙 챗봇에서 시작해 읽기 → 쓰기 → 기억 → 관찰 → 안전장치 → 동료 Agent까지, 단계(stage)를 하나씩 밟으며 실제 업무를 통째로 맡기는 과정을 보여줍니다.
...
`stage6_multiagent`의 무인 완주 시연 전에 다음 주 배치(week2)를 투입하려면:
...
| `stage3_memory` | 실험 노트 파일 하나로 지난 작업 맥락을 기억한다 |
| `stage4_loop` | qPCR 실행을 지켜보며 이상 신호를 스스로 감지한다 |
...
기본입니다(대사가 화면에도 그대로 보이도록). 슬래시 커맨드도 동일한 프롬프트로 동작하니
시작 킷을 직접 실습할 때 편한 쪽을 쓰면 됩니다.

파일명 앞 숫자는 발표 전체 타임라인 기준 전역 번호이며, 같은 커맨드는 모든 stage에서 같은
번호·파일명을 유지합니다.

- `stage0_chatbot`: `/1_해석` — 이번 주 qPCR 결과 CSV를 해석해 달라고 요청 (기기 질문은 커맨드 없이 직접 쳐서 "모른다"를 보여줌)
- `stage1_read`: `/2_기기확인` — 지금 QuantStudio 장비가 비어 있는지 물어봄 · `/3_합격샘플` — 순도 기준(QC)을 넘는 샘플만 골라 달라고 요청
- `stage2_write`: `/2_기기확인` · `/3_합격샘플` · `/4_플레이트준비` — 합격 샘플을 희석해 플레이트에 준비해 달라고 요청 (승인 필요)
- `stage3_memory`: `/2_기기확인` · `/3_합격샘플` · `/4_플레이트준비` · `/5_지난주처럼` — 새 세션에서 지난주와 같은 방식으로 준비해 달라고 요청 (메모 활용 확인)
- `stage4_loop`: `/2_기기확인` · `/3_합격샘플` · `/4_플레이트준비` · `/5_지난주처럼` · `/6_qpcr실행` — qPCR 런을 시작하고 끝날 때까지 지켜봐 달라고 요청 (피드백 루프)
- `stage5_guardrail`: `/2_기기확인` · `/3_합격샘플` · `/4_플레이트준비` · `/5_지난주처럼` · `/6_qpcr실행` · `/7_전량재준비` — 전 샘플을 5배 부피로 다시 준비해 달라고 요청 (시약 한계 초과 시나리오)
- `stage6_multiagent`: `/2_기기확인` · `/3_합격샘플` · `/4_플레이트준비` · `/5_지난주처럼` · `/6_qpcr실행` · `/7_전량재준비` · `/8_무인완주` — 새 배치 검사부터 보고서 작성까지 전체를 위임 (동료 Agent가 채점)
